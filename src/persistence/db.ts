...
function _migrationAlreadyApplied(database: Database, version: number): boolean {
...
const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    sql: `
      ...
      -- Copy data, mapping original implicit rowid to the new explicit rowid
      ...
      -- Drop the old table first so we can reuse index names
      ...
      -- Create proposals index after old table is dropped
      CREATE INDEX idx_proposals_project_status ON memory_proposals(project_id, status);
    `,
  },
  {
  ...
  },
  {
    ...
    sql: `
      -- Migration v3 renames memory_items while rebuilding rowid. SQLite also
      -- rewrites existing dependent foreign keys during that rename, so repair
      -- graph/link tables to point back at the rebuilt memory_items table.
